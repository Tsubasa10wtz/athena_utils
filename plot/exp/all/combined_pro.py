...
for j, model in enumerate(model_names_jct):
    ax1.bar(
        ...
        label=model,
        zorder=3,
        color=colors[j % len(colors)],  # 设置柱体颜色
        hatch=hatches[j % len(hatches)],  # 设置底纹
...
ax1.set_yticks(np.arange(0, 7, 1))  # 设置刻度间隔为 0.3
...
fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 1.05),
           ncol=max(len(model_names_jct), len(model_names_chr)), fontsize=46, frameon=False)
